...
for     no =    1:node_num-1
%% A.Drag Force
    Fdn     =   -0.5*Cdn*rho_water*D*l(no)*norm(vN(no,:))*vN(no,:);%切線
    Fdt     =   -0.5*Cdt*rho_water*D*l(no)*norm(vT(no,:))*vT(no,:);%法線
    Fd      =   Fdn+Fdt;%總阻力   
    
    FD(1+(no-1)*3:3+(no)*3)     =   FD(1+(no-1)*3:3+(no)*3)  +   0.5*[Fd Fd]';%阻力矩陣更新
