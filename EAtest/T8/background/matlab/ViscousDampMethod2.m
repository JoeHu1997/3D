...
while convergence  ==  0
    ...
    deltaR = tR     -   tF;
    [K_L  Ma]  =    KL(Ca,k,EA,order,ele_num,ele_info,ele_he,ele_cos,rho_water,Area,timer2);
...
%     K_NL(1:6,:) = 0;
    ...
    if first    ==  1
        botten_node= [];
        deltaU1 =   displacement(K,node_num,fix_pt_node,deltaR,float_pt_node,dir,botten_node);
        %deltaU1(3)  = 0;
        deltaU  =   deltaU1;        
        R0      =   deltaR;
        first   =   0;
        ...
        botten_node = [];
        deltaU  =   displacement(K,node_num,fix_pt_node,deltaR,float_pt_node,dir,botten_node);
        %deltaU(3:3:(timer2+1)*3)  = 0;
        
    end
    
    tU          =   tU  +   deltaU;
    convergence = E_conv(R0,deltaR,deltaU,deltaU1,RNORM,ETOL,RTOL,DTOL,fix_pt_node,float_pt_node,botten_node,timer1);
